      ...
      authenticated ? (
        <div className="max-sm:relative">
          ...
          <div className="">
           ...
           <div className={`max-sm:w-full max-sm:absolute max-sm:p-10 w-full transition-all ease-in duration-300 ${ userConnected ? "inset-0 bg-opacity-50 backdrop-blur" : ""} `}>
           {
            userConnected ? (
            <>
            <p className="flex items-center justify-end mb-3"><RiCloseCircleLine size={30} onClick={() => setUserConnected(false)}/></p>
            <div className={`w-full h-[70vh] p-3 bg-green-300 shadow-2xl overflow-hidden transition-all ease-in duration-300 ${ userConnected ? "scale-100 opacity-100" : "scale-95 opacity-0"}`}>
